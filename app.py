"""
Trading Scanner v3 – Entry Precision + KO-Zertifikat Panel (refactored + Regeln)
...
def evaluate_rules(df: pd.DataFrame, direction: str, price: float, atr: float, market: dict):
    ...
    if direction == "LONG":
        if not (price and ema20 and ema50 and ema200 and price > ema20 > ema50 > ema200):
            ok = False
            reasons.append("Trend nicht klar: Kurs/EMAs nicht in sauberer Long-Struktur (TRADEN WENN: Trend klar).")
    else:
        if not (price and ema20 and ema50 and ema200 and price < ema20 < ema50 < ema200):
            ok = False
            reasons.append("Trend nicht klar: Kurs/EMAs nicht in sauberer Short-Struktur (TRADEN WENN: Trend klar).")

    # TRADEN WENN: Volatilität moderat (ATR normal, keine News)
    if atr_pct is None or not (0.5 <= atr_pct <= 3.0):
        ok = False
        reasons.append(f"ATR% nicht moderat ({atr_pct:.2f if atr_pct else 'n/a'}) – Volatilität zu niedrig/hoch.")
    ...
    if rsi is None or macd is None or msig is None or not (45 <= rsi <= 60 and macd > msig):
        ok = False
        reasons.append(f"Momentum nicht ideal (RSI {rsi}, MACD/Signal) – TRADEN WENN: Momentum positiv verletzt.")

    # NICHT TRADEN WENN: Trend bricht (unter EMAs, Range)
    if direction == "LONG" and price and ema20 and price < ema20:
        ok = False
        reasons.append("Trendbruch: Kurs unter EMA20 – NICHT TRADEN WENN: Trend bricht.")
    if direction == "SHORT" and price and ema20 and price > ema20:
        ok = False
        reasons.append("Trendbruch: Kurs über EMA20 – NICHT TRADEN WENN: Trend bricht.")

    # NICHT TRADEN WENN: Volatilität hoch (VIX>20)
    if vix is not None and vix > 20:
        ok = False
        reasons.append(f"VIX hoch ({vix:.1f}) – NICHT TRADEN WENN: Volatilität hoch (VIX>20).")

    # NICHT TRADEN WENN: Momentum negativ (RSI <40 oder >70)
    if rsi is not None and (rsi < 40 or rsi > 70):
        ok = False
        reasons.append(f"Momentum negativ (RSI {rsi:.1f}) – NICHT TRADEN WENN: Momentum negativ.")

    # AKTIE ERFÜLLT VORAUSSETZUNGEN: Trendstruktur stimmt (EMA20>EMA50>EMA200)
    if direction == "LONG":
        if not (ema20 and ema50 and ema200 and ema20 > ema50 > ema200):
            ok = False
            reasons.append("Trendstruktur unsauber (EMA20>EMA50>EMA200 fehlt) – AKTIE ERFÜLLT VORAUSSETZUNGEN NICHT.")
    else:
        if not (ema20 and ema50 and ema200 and ema20 < ema50 < ema200):
            ok = False
            reasons.append("Trendstruktur unsauber (EMA20<EMA50<EMA200 fehlt) – AKTIE ERFÜLLT VORAUSSETZUNGEN NICHT.")
    ...
    if not good_market:
        ok = False
        reasons.append("Marktumfeld nicht ideal (SPY/QQQ nicht grün oder VIX nicht niedrig) – AKTIE ERFÜLLT VORAUSSETZUNGEN NICHT.")
...
styled = (
    ...
    .set_properties(**{"background-color":"#ffffff","color":"#111827","border-color":"#e5e7eb"})
)
